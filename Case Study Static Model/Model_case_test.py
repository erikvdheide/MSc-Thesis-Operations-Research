...
# Flows between grids
Q_pilgg = Model.addMVar(shape=(len(P), len(I), len(L), len(G), len(G)), lb=0.0)
...
Model.addConstrs(Q_pilgg[(p, i, l, g, g_prime)] == 0 for p in P for i in I for l in L for g in G for g_prime in G if A_il[(i, l)] == 0 or A_ig[(i, g)] == 0)
...
Model.addConstrs(DL_ig[(i, g)] == gp.quicksum(Q_pilgg[(p, i, l, g, g)] for p in P for l in L if A_il[(i, l)] == 1) for i in I for g in G)

# (B.3) Imported hydrogen (has NO constraint on location)
Model.addConstrs(DI_ig[(i, g)] == gp.quicksum(Q_pilgg[(p, i, l, g_prime, g)] for p in P for l in L for g_prime in G if g_prime != g) for i in I for g in G)
...
Model.addConstrs(DL_ig[(i, g)] == PT_ig[(i, g)] - gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P for l in L for g_prime in G if g_prime != g) for i in I for g in G)
...
Model.addConstrs(gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P) >= QMin_il[(i, l)] * X_ilgg[(i, l, g, g_prime)] for i in I for l in L for g in G for g_prime in G
                 if A_il[(i, l)] == 1 and A_ig[(i, g)] == 1)
Model.addConstrs(gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P) <= QMax_il[(i, l)] * X_ilgg[(i, l, g, g_prime)] for i in I for l in L for g in G for g_prime in G
                 if A_il[(i, l)] == 1 and A_ig[(i, g)] == 1)
...
Model.addConstrs(NTU_ilgg[(i, l, g, g_prime)] >= gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P) * (1 / TMA_l[l]) * (1 / TCap_il[(i, l)]) * (2 * L_lgg[(l, g, g_prime)] * (1 / SPbetween_l[l]) + LUT_l[l])
...
Model.addConstrs(NTU_ilgg[(i, l, g, g)] >= gp.quicksum(Q_pilgg[(p, i, l, g, g)] for p in P) * (1 / TMA_l[l]) * (1 / TCap_il[(i, l)]) * (2 * L_lgg[(l, g, g)] * (1 / SPwithin_l[l]) + LUT_l[l])
...
# (B.22) Fuel Cost (FC)
Model.addConstr(FC == gp.quicksum(FP_l[l] * (2 * L_lgg[(l, g, g_prime)] * gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P) * (1 / FEbetween_l[l]) * (1 / TCap_il[(i, l)]))
                                  for i in I for l in L for g in G for g_prime in G if A_il[(i, l)] == 1 if g_prime != g)
                + gp.quicksum(FP_l[l] * (2 * L_lgg[(l, g, g)] * gp.quicksum(Q_pilgg[(p, i, l, g, g)] for p in P) * (1 / FEwithin_l[l]) * (1 / TCap_il[(i, l)]))
                              for i in I for l in L for g in G if A_il[(i, l)] == 1))

# (B.23) Labour Cost (LC)
Model.addConstr(LC == gp.quicksum(DW_l[l] * (gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P) * (1 / TCap_il[(i, l)]) * (2 * L_lgg[(l, g, g_prime)] * (1 / SPbetween_l[l]) + LUT_l[l]))
                                  for i in I for l in L for g in G for g_prime in G if A_il[(i, l)] == 1 if g_prime != g)
                + gp.quicksum(DW_l[l] * (gp.quicksum(Q_pilgg[(p, i, l, g, g)] for p in P) * (1 / TCap_il[(i, l)]) * (2 * L_lgg[(l, g, g)] * (1 / SPwithin_l[l]) + LUT_l[l]))
                              for i in I for l in L for g in G if A_il[(i, l)] == 1))

# (B.24) Maintenance Cost (MC)
Model.addConstr(MC == gp.quicksum(ME_l[l] * (2 * L_lgg[(l, g, g_prime)] * gp.quicksum(Q_pilgg[(p, i, l, g, g_prime)] for p in P) * (1 / TCap_il[(i, l)]))
...
if optimizeCI:
    ...
    # (B.45) CI^{Prod}_{ig}
    Model.addConstrs(CIProd_ig[(i, g)] * gp.quicksum(Q_pilgg[(p_star, i, l_star, g_prime_star, g)] for p_star in P for l_star in L for g_prime_star in G)
                     == gp.quicksum(Q_pilgg[(p, i, l, g_prime, g)] * CIStart_pi[(p, i)] for l in L for g_prime in G) for i in I for g in G)
...
for i in I:
    for g in G:
        CIProd_denominator_ig[(i, g)] = gp.quicksum(Q_pilgg[(p_star, i, l_star, g_prime_star, g)] for p_star in P for l_star in L for g_prime_star in G).getValue()
for i in I:
    for g in G:
        if CIProd_denominator_ig[(i, g)] > 0.0000001:
            CIProd_ig_post[(i, g)] = gp.quicksum(Q_pilgg[(p, i, l, g_prime, g)] / CIProd_denominator_ig[(i, g)] * CIStart_pi[(p, i)] for l in L for g_prime in G).getValue()
...
if printVariables:
    ...
    for p in P:
        for i in I:
            for l in L:
                if A_il[(i, l)] == 1:
                    matrix = Q_pilgg[p, i, l, :, :].X.round(2)
                    if not deleteZeroMatrix(matrix):
                        print(f"Q_[{PSet[p]}]{ISet[i]}][{LSet[l]}]gg' (plant type {PSet[p]} transport product {ISet[i]} using mode {LSet[l]}): "
                              f"\n {pd.DataFrame(deleteZeroElements(Q_pilgg[p, i, l, :, :].X.round(2)), index=GSet, columns=GSet).to_string()}")
                        print()
