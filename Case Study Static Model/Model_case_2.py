...
if optimizeCI:
    ...
    # (B.46) Calculate the starting carbon intensity as parameter per plant p per product i
    ...
    # (B.52) CI_{ig} restrictions
    for i in I:
        for g in G:
            if maxCI[(i, g)] != -1:
                Model.addConstr(CI_ig[(i, g)] <= maxCI[(i, g)])

    # (B.53) CI_{g} restrictions
    for g in G:
        if maxCI[(len(I), g)] != -1:
...
# (B.41) Carbon intensity from transportation
...
# (B.42) Total carbon intensity is part from production and part from transportation
CI_ig_post = np.zeros(shape=(len(I), len(G)))
for i in I:
    for g in G:
        CI_ig_post[(i, g)] = CIProd_ig_post[(i, g)] + CITrans_ig_post[(i, g)]

# (B.43) Carbon intensity at a location is the resource-weighted average of both products
CI_g_post = np.zeros(shape=len(G))
for g in G:
    CI_g_post[g] = gp.quicksum(CI_ig_post[(i, g)] * (DL_ig[(i, g)] + DI_ig[(i, g)]) / DT_g[g] for i in I).getValue()

# (B.44) Total carbon emissions for which grid g is responsible, per product
Carbon_ig_post = np.zeros(shape=(len(I), len(G)))
for i in I:
    for g in G:
        Carbon_ig_post[(i, g)] = CI_ig_post[(i, g)] * (DL_ig[(i, g)].X + DI_ig[(i, g)].X)

# (B.45) Total carbon emissions for which grid g is responsible, all products combined
...
if printVariables:
    ...
    FeedNeeded_p = np.zeros(shape=math.floor(len(P)/2))  # general if all technologies can have CCS
    indices = ["Gas", "Coal", "Biomass", "Electricity"]
    for p in P:
        FeedNeeded_p_large[p] = (PEF_p[p] * gp.quicksum(P_pig[(p, i, g)] for i in I for g in G)).getValue()
    for p in range(math.floor(len(P)/2)):
        FeedNeeded_p[p] = FeedNeeded_p_large[p] + FeedNeeded_p_large[p+math.floor(len(P)/2)]
    print("FeedNeeded_p \n", pd.DataFrame(deleteZeroElements(FeedNeeded_p), index=indices, columns=["Amount of feed"]).to_string())
    print()
...
if optimizeCI:
    ...
    print(pd.DataFrame(CI_and_Carbon, index=GSet, columns=["CI_[CH2]g", "CI_[LH2]g", "CI_g", "Carbon_[CH2]g", "Carbon_[LH2]g", "Carbon_g"]).to_string())
    print()
    print("Validity check DURING-optimization: ")
    print("TCE (right)        =", round(TCE.X, 3))
    print("sum(Carbon_g)      =", round(sum(CI_g.X * DT_g), 3))
    print()

print("Validity check POST-optimization: ")
